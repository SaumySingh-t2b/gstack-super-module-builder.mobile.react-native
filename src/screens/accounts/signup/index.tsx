...
import {Layout, Input, Button, Text, CheckBox} from '@ui-kitten/components'
...
export const SignUp = ({navigation}: AuthScreenProps<'Signup'>) => {
  ...
  const switchTermsSelection = useCallback(
    (isChecked: boolean) => setTermsAccepted(isChecked),
    [],
  )

  const switchPolicySelection = useCallback(
    (isChecked: boolean) => setPolicyAccepted(isChecked),
    [],
  )
